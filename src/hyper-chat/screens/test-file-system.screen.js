...
class TestFileSystemScreen extends React.Component {
    ...
    assert = (name, val, exp) => {
        if (exp !== val) {
            throw new Error(name + ': "' + String(val) + '" should be "' + String(exp) + '"');
        }
...
const connectedToTestFileSystemScreen = connect(mapState, actionCreators)(TestFileSystemScreen);
export {connectedToTestFileSystemScreen as TestFileSystemScreen};
